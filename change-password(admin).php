...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (strlen($newPassword) != 6) {
        $message = "New password need to be 6 characters.";
        ...
        if ($storedPassword !== $oldPassword) {
            ...
            mysqli_stmt_bind_param($updateStmt, "ss", $newPassword, $ID);
            $updateSuccess = mysqli_stmt_execute($updateStmt);
            mysqli_stmt_close($updateStmt);

            if ($updateSuccess) {
                echo "<script>alert('Password changed successfully!'); window.location.href='view-profile(admin).php';</script>";
                exit();
            } else {
